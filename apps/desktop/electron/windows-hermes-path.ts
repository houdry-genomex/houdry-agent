 ...
 *      and Houdry launcher. Marker-only or partial runtimes must use --repair;
 ...
 * venv Python interpreter and the Houdry launcher that drives `hermes update`.
...
export function resolveVenvHermesCommand(
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Houdry at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Houdry Python at ${python}`,
    command: python,
