 ...
 * preview servers. Unknown Python/Houdry processes are deliberately ignored.
 */
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Houdry process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Houdry installation is free.\n' +
    '\n' +
    'Close other Houdry windows and terminals, then retry.  If the problem\n' +
