...
// The Nous portal (NAS) does NOT use Houdry gateway session cookies — it is a
...
/** Which Houdry profile the remote SSH dashboard should actually run as.
...
// Houdry Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  // name used by the remote Houdry installation. Preserve an explicit mapping
  // when it is a valid Houdry profile identifier; otherwise fall back to the
 ...
 * the Houdry gateway session cookies, so cloud sign-in / discovery liveness
