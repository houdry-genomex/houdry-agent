...
async function probeWindowsRemote(ssh, explicitHermesPath = '') {
  ...
  const script = [
    ...
    'if(-not $hermes){throw "Houdry is not installed on the remote Windows host."}',
    'Assert-NoReparse $hermes $false',
    'if($explicit -and $hermes -ne $explicit){throw "The configured Houdry path is not an executable file."}',
...
async function connectWindowsRemote(deps) {
  ...
  if (!inspection.supported) {
    const error: any = new Error('Update Houdry on the remote Windows host before connecting with Desktop SSH.')
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Houdry SSH"', 'powershell.exe -NoLogo')
