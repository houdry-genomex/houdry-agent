 ...
 *
 *   macOS:   …/Houdry.app/Contents/MacOS/Houdry  → …/Houdry.app
 *   Windows: …\Houdry\Houdry.exe                 → …\Houdry  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Houdry.app/Contents/MacOS/Houdry → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Houdry.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Houdry.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Houdry$/i.test(dir) || /[\\/]hermes-desktop$/i.test(dir)) {
      return dir
    }
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Houdry install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
