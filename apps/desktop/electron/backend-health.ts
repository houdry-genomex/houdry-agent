...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Houdry gateway uses OAuth, but you are not signed in. ' +
...
/**
 * True when the backend URL points at a Nous-managed Houdry Cloud instance
...
export async function waitForHermesReady(baseUrl: string, options: HermesReadyOptions): Promise<void> {
  ...
  throw new Error(`Houdry backend did not become ready: ${detail}`)
}
