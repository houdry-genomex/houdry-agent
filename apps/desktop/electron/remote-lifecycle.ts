 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Houdry dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Houdry install on the remote (login-shell probe),
...
async function locateHermes(ssh, remoteHermesPath) {
  ...
  if (remoteHermesPath) {
    ...
    const err: any = new Error(
      `The Houdry path you set is not an executable on the remote host: "${remoteHermesPath}". ` +
  ...
  const err: any = new Error(
    'Houdry is not installed on the remote host (could not find a `hermes` executable). ' +
      'Install it on the remote with:  curl -fsSL https://raw.githubusercontent.com/houdry-genomex/houdry-agent/main/scripts/install.sh | sh  ' +
      '— or set the Houdry path explicitly in the SSH connection settings.'
  )
...
// e.g. "Houdry Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH hermes a
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Houdry Agent SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
async function probeRemoteHermesHome(ssh) {
  try {
    ...
    const error: any = new Error('Could not resolve the remote Houdry home.')
 ...
 * bytes; it never imports or executes code from the changing Houdry checkout.
...
async function assertRemoteInstallUpdateClear(ssh, hermesHome) {
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Houdry install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Houdry update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Houdry update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteHermesProfiles(ssh) {
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Houdry profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Houdry home.')
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, hermesPath))) {
    const err: any = new Error(
      'The remote Houdry install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Houdry on the remote host to continue using Desktop SSH mode.'
    )
...
async function connect(deps) {
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Houdry Agent build (${lock.reason}). ` +
