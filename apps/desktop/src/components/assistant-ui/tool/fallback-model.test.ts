...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Houdry Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Houdry docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Houdry Agent Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Houdry docs', url: 'https://example.com/docs' }
    ])
  })
})
