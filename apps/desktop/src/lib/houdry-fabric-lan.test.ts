...
describe('fabricHost', () => {
  it('extracts host:port', () => {
    expect(fabricHost('https://192.168.1.10:8080/v1')).toBe('192.168.1.10:8080')
  })
...
describe('shouldAdoptDiscoveredUrl', () => {
  const placeholder = 'https://127.0.0.1:18080/v1'
  ...
  it('does not overwrite a URL the user typed', () => {
    expect(shouldAdoptDiscoveredUrl('https://10.0.0.8:8080/v1', placeholder)).toBe(false)
  })
})
...
describe('mergeFabricLanScan', () => {
  it('uses this computer when loopback answers, even if WiFi also advertised', () => {
    ...
    const out = mergeFabricLanScan([wifi], 'https://127.0.0.1:8090/v1')

    expect(out).toHaveLength(1)
    expect(out[0]).toMatchObject({
      api: 'https://127.0.0.1:8090/v1',
  ...
  it('keeps WiFi ads when this computer is not serving', () => {
    ...
    expect(mergeFabricLanScan([wifi], null).map(ep => ep.api)).toEqual(['https://192.168.1.10:8080/v1'])
  })
})
...
describe('displayNameFor', () => {
  it('labels loopback as this computer', () => {
    expect(displayNameFor(fromLoopbackHit('https://127.0.0.1:8080/v1'), 'This computer')).toBe('This computer')
  })
